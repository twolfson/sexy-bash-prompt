...
# prompt status symbols
  ...
  PROMPT_SYNCED_SYMBOL='#synced' PROMPT_DIRTY_SYNCED_SYMBOL='#dirty-synced' \
    ...
    fixture_dir 'synced'
    test "$(get_git_status)" = "#synced" || echo '`get_git_status` !== "#synced" on an overridden synced branch' 1>&2

    # a dirty synced prompt uses the new symbol
    fixture_dir 'dirty-synced'
    test "$(get_git_status)" = "#dirty-synced" || echo '`get_git_status` !== "#dirty-synced" on an overridden dirty synced branch' 1>&2

    # an unpushed prompt uses the new symbol
    fixture_dir 'unpushed'
    test "$(get_git_status)" = "#unpushed" || echo '`get_git_status` !== "#unpushed" on an overridden and unpushed branch' 1>&2

    # a dirty and unpushed prompt uses the new symbol
    fixture_dir 'dirty-unpushed'
    test "$(get_git_status)" = "#dirty-unpushed" || echo '`get_git_status` !== "#dirty-unpushed" on an overridden, dirty, and unpushed branch' 1>&2

    # an unpulled prompt uses the new symbol
    fixture_dir 'unpulled'
    test "$(get_git_status)" = "#unpulled" || echo '`get_git_status` !== "#unpulled" on an overridden and unpulled branch' 1>&2

    # a dirty and unpulled prompt uses the new symbol
    fixture_dir 'dirty-unpulled'
    test "$(get_git_status)" = "#dirty-unpulled" || echo '`get_git_status` !== "#dirty-unpulled" on an overridden, dirty, and unpulled branch' 1>&2

    # an unpushed and unpushed prompt uses the new symbol
    fixture_dir 'unpushed-unpulled'
    test "$(get_git_status)" = "#unpushed-unpulled" || echo '`get_git_status` !== "#unpushed-unpulled" on an overridden, unpushed, and unpulled branch' 1>&2

    # a dirty, unpushed, and unpushed prompt uses the new symbol
    fixture_dir 'dirty-unpushed-unpulled'
    test "$(get_git_status)" = "#dirty-unpushed-unpulled" || echo '`get_git_status` !== "#dirty-unpushed-unpulled" on an overridden, dirty, unpushed, and unpulled branch' 1>&2
